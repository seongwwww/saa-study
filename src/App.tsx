...
import { BookOpen, ChevronLeft, ChevronRight, Cloud, Home, RotateCcw, Bookmark, History, Settings, CheckCircle2, ClipboardList, ArrowRight, Type, X, AlertCircle, Shuffle, GraduationCap } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(()=>{
    if(!ready)return;
    const check=()=>{const current=stateRef.current;const expired=expireMockExams(current,questions);if(expired===current)return;setState(s=>expireMockExams(s,questions));const finished=current.sessions.find(s=>s.id===current.activeSessionId&&!s.submittedAt&&s.mode==='mock'&&Date.parse(s.mock!.deadlineAt)<=Date.now());if(finished){setResultId(finished.id);setView('results');setConfirm(null);setNotice('제한 시간이 끝나 모의고사를 자동 제출했습니다.');}};
  ...
  function openStudy(index:number) {setState(s=>({...s,studyIndex:index,studyDraft:index===s.studyIndex?s.studyDraft:undefined}));setGridPage(Math.floor((stateRef.current.studyOrder?stateRef.current.studyOrder.indexOf(questions[index].id):index)/50));navigate('study');}
  function shuffleStudy() {
    const begin=()=>{setState(s=>startRandomStudy(s,questions));setGridPage(0);navigate('study');};
    if(state.studyOrder)setConfirm({title:'전체 문제를 다시 섞을까요?',body:'저장된 메모와 풀이 기록은 유지되고, 랜덤 학습 순서와 현재 답안만 새로 시작합니다.',label:'다시 섞기',action:begin});else begin();
  }
  ...
  function startSession(ids:string[],mode:Session['mode'],mockDomains?:NonNullable<Session['mock']>['domains']) {
    if(!ids.length){setNotice('선택한 조건에 해당하는 문제가 없습니다.');return;}
    const begin=()=>{const session:Session={id:crypto.randomUUID(),mode,ids,answers:{},position:0,startedAt:new Date().toISOString()};if(mode==='mock'&&mockDomains)session.mock={durationMinutes:ids.length*2,deadlineAt:new Date(Date.parse(session.startedAt)+ids.length*2*60000).toISOString(),domains:mockDomains};setState(s=>({...s,sessions:[...s.sessions,session],activeSessionId:session.id}));navigate('quiz');};
  ...
  function startMock(count:number) {try{const exam=buildMockExam(questions,state,count);startSession(exam.ids,'mock',exam.domains);}catch(err){setNotice(err instanceof Error?err.message:'출제할 문제를 확인해 주세요.');}}
  ...
  const currentLabel=view==='settings'?'설정 및 백업':view==='results'?'풀이 결과':view==='quiz'?(active?.mode==='mock'?'모의고사':'문제 풀기'):navigation.find(x=>x[0]===view)?.[2];
  const q=questions[state.studyIndex],draft=state.studyDraft?.qid===q.id?state.studyDraft:undefined;
  const order=studySequence(questions,state), studyPosition=order.indexOf(q.id);
  const questionsById=new Map(questions.map(question=>[question.id,question]));
  const orderedQuestions=order.map(id=>questionsById.get(id)!);
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-icon"><BookOpen size={21}/></span><div>SAA <span>Study</span><small>나의 AWS 공부방</small></div></div><div className="nav-caption">WORKSPACE</div><nav aria-label="주 메뉴">{navigation.map(([key,Icon,label])=><button key={key} className={view===key||(key==='mock'&&(view==='quiz'?active?.mode==='mock':view==='results'&&state.sessions.find(s=>s.id===resultId)?.mode==='mock'))||(key==='setup'&&(view==='quiz'?active?.mode!=='mock':view==='results'&&state.sessions.find(s=>s.id===resultId)?.mode!=='mock'))?'active':''} onClick={()=>navigate(key)}><Icon size={18}/>{label}{key==='review'&&stats.review.length>0&&<span className="nav-count">{stats.review.length}</span>}</button>)}<button className={`settings-nav ${view==='settings'?'active':''}`} onClick={()=>navigate('settings')}><Settings size={18}/>설정 및 백업</button></nav><div className="sidebar-bottom"><Cloud size={18}/><span>나의 속도로, 한 문제씩.<small>AWS SAA-C03 · {questions.length} questions</small></span></div></aside>
    ...
    {view==='study'&&<><PageHeading eyebrow="LEARN AT YOUR PACE" title="한 문제씩, 확실하게." description="먼저 생각하고, 정답과 해설을 확인해 보세요." action={fontControls}/><div className="study-order-bar"><div><strong>{state.studyOrder?'랜덤 학습':'번호순 학습'}</strong><span>{state.studyOrder?`중복 없이 ${questions.length}문제 · 순서와 위치 자동 저장`:'문제 번호 순서대로 차근차근 공부해요.'}</span></div><div><button className="button secondary" disabled={!state.studyOrder} onClick={()=>{setState(s=>({...s,studyOrder:undefined}));setGridPage(Math.floor(state.studyIndex/50));}}>번호순</button><button className="button primary" onClick={shuffleStudy}><Shuffle size={16}/>{state.studyOrder?'전체 다시 섞기':`전체 ${questions.length}문제 섞기`}</button></div></div><div className="study-layout study-with-notes"><div><div className="study-toolbar"><span>{state.studyOrder?'랜덤 순서':'전체 문제'} <b>{studyPosition+1} / {questions.length}</b>{state.studyOrder&&<small> · 원문 Q{q.number}</small>}</span><form className="jump-form" onSubmit={e=>{e.preventDefault();const n=Number(jump);if(Number.isInteger(n)&&n>=1&&n<=questions.length)openStudy(n-1);else setNotice(`1~${questions.length} 사이 번호를 입력해 주세요.`);}}><label htmlFor="jump">문제 이동</label><input id="jump" type="number" min={1} max={questions.length} value={jump} onChange={e=>setJump(e.target.value)}/><button type="submit" aria-label="입력한 문제로 이동"><ArrowRight size={15}/></button></form></div>
      ...
      <div className="bottom-navigation"><button className="button secondary" disabled={!studyPosition} onClick={()=>moveStudy(-1)}><ChevronLeft size={16}/>이전 문제</button>{draft?.graded&&<button className="text-button" onClick={()=>{setState(s=>({...s,studyDraft:undefined}));window.scrollTo(0,0);}}><RotateCcw size={14}/>다시 풀기</button>}<button className="button primary" disabled={studyPosition===questions.length-1} onClick={()=>moveStudy(1)}>다음 문제<ChevronRight size={16}/></button></div><section className="panel study-navigator" aria-label="문제 둘러보기"><div className="section-eyebrow">QUESTION NAVIGATOR</div><div className="panel-title"><h3>{state.studyOrder?'랜덤 순서 둘러보기':'문제 둘러보기'}</h3><select aria-label={state.studyOrder?'랜덤 순서 범위':'문제 번호 범위'} value={gridPage} onChange={e=>setGridPage(Number(e.target.value))}>{Array.from({length:Math.ceil(questions.length/50)},(_,i)=><option key={i} value={i}>{i*50+1}–{Math.min((i+1)*50,questions.length)}{state.studyOrder?'번째':''}</option>)}</select></div><div className="question-grid">{orderedQuestions.slice(gridPage*50,(gridPage+1)*50).map(x=>{const status=stats.status.get(x.id);return <button key={x.id} aria-label={`${x.number}번 문제`} aria-current={q.id===x.id?'true':undefined} className={`${q.id===x.id?'current':''} ${status?.needsReview?'wrong':status?.latest?.correct?'done':''}`} onClick={()=>openStudy(x.number-1)}>{x.number}</button>;})}</div><div className="grid-legend"><span>□ 미풀이</span><span className="green-text">□ 최근 정답</span><span className="red-text">□ 복습 필요</span></div></section></div>
      <aside className="study-notes" aria-label="문제 필기"><div className="panel note-panel"><div className="section-eyebrow">MY STUDY NOTES</div><div className="note-question">문제 {String(q.number).padStart(3,'0')} · 나의 필기</div><QuestionNote question={q} progress={state.progress[q.id]||{}} onProgress={patch=>progress(q.id,patch)}/><p className="microcopy">문제를 풀면서 바로 적어보세요.<br/>필기는 문제별로 자동 저장됩니다.</p><DomainPicker question={q} progress={state.progress[q.id]||{}} onProgress={patch=>progress(q.id,patch)}/></div></aside></div></>}
    {view==='setup'&&<SessionSetup questions={questions} state={state} active={active} onStart={startSession} onResume={()=>navigate('quiz')}/>}
    {view==='mock'&&<MockExamSetup questions={questions} state={state} active={active} onStart={startMock} onResume={()=>navigate('quiz')} onProgress={progress} onStudy={openStudy}/>}
    {view==='quiz'&&(active?<><PageHeading eyebrow={active.mode==='mock'?'SAA-C03 PRACTICE EXAM':active.mode==='review'?'RECALL & REVIEW':'FOCUS SESSION'} title={active.mode==='mock'?'모의고사, 끝까지 집중해요.':active.mode==='review'?'다시 풀며, 내 것으로.':'지금은 문제에만 집중해요.'} description="선택한 답은 자동 저장됩니다. 정답은 세트 제출 후 공개됩니다." action={<div className="exam-header-tools">{active.mock&&<ExamClock deadlineAt={active.mock.deadlineAt}/>} {fontControls}</div>}/><div className="study-layout"><div><div className="study-toolbar"><span>{active.mode==='mock'?'모의고사':active.mode==='review'?'오답 복습':'문제풀이'} <b>{active.position+1} / {active.ids.length}</b></span><button className="text-button" onClick={()=>navigate('home')}>{active.mock?'저장하고 나가기 · 시간은 계속 진행':'저장하고 나가기'}</button></div>{(()=>{const current=questions.find(x=>x.id===active.ids[active.position])!;return <QuestionCard question={current} selected={active.answers[current.id]||[]} revealed={false} onSelect={key=>setState(s=>({...s,sessions:s.sessions.map(session=>session.id===active.id?{...session,answers:{...session.answers,[current.id]:toggleAnswer(current,session.answers[current.id]||[],key)}}:session)}))} progress={state.progress[current.id]||{}} onProgress={patch=>progress(current.id,patch)} fontSize={state.fontSize}/>;})()}<div className="bottom-navigation"><button className="button secondary" disabled={!active.position} onClick={()=>{setState(s=>({...s,sessions:s.sessions.map(x=>x.id===active.id?{...x,position:x.position-1}:x)}));window.scrollTo(0,0);}}><ChevronLeft size={16}/>이전</button>{active.position<active.ids.length-1?<button className="button primary" onClick={()=>{setState(s=>({...s,sessions:s.sessions.map(x=>x.id===active.id?{...x,position:x.position+1}:x)}));window.scrollTo(0,0);}}>다음 문제<ChevronRight size={16}/></button>:<button className="button primary" onClick={completeSession}>제출하고 결과 보기<CheckCircle2 size={16}/></button>}</div></div><aside className="study-rail"><div className="panel"><div className="section-eyebrow">SESSION PROGRESS</div><h3>{active.ids.filter(id=>active.answers[id]?.length).length} <span className="muted">/ {active.ids.length}문제 응답</span></h3><div className="progress-track"><span style={{width:`${active.ids.filter(id=>active.answers[id]?.length).length/active.ids.length*100}%`}}/></div><div className="question-grid">{active.ids.map((id,i)=><button key={id} aria-label={`세트 ${i+1}번째 문제`} className={`${i===active.position?'current':''} ${active.answers[id]?.length?'answered':''}`} onClick={()=>{setState(s=>({...s,sessions:s.sessions.map(x=>x.id===active.id?{...x,position:i}:x)}));window.scrollTo(0,0);}}>{i+1}</button>)}</div><button className="button primary full-width" onClick={completeSession}>제출하고 결과 보기</button><p className="microcopy">비어 있는 번호는 아직 답을 고르지 않은 문제입니다.</p></div></aside></div></>:<div className="empty-state"><ClipboardList size={32}/><h2>진행 중인 세트가 없어요.</h2><button className="button primary" onClick={()=>navigate('setup')}>새 세트 만들기</button></div>)}
